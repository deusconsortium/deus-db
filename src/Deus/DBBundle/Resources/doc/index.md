DEUS DATABASE
=============
by Jean Pasdeloup - 2016-23-03 - Draft
...
The basic feature of deus-db is to allow simple search among ObjectGroups, ie the files generated by the simulations.

It is composed of a dynamic table filterable by each of the colums using selectors on top of them. 
Those selectors are dynamic and only display available values for the current selection.
A reset button allows to remove all the filters and see the whole list.
...
This interface allows the user to edit the settings and datas.
